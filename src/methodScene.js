var methodLayer=cc.Layer.extend({
    ctor:function(){
        ...
        // 游戏方法文字内容
        var text = new ccui.Text("两人轮流翻牌，根据翻出的卡牌计分。每翻完一张卡牌，即时结算。翻完所有卡牌后，分数最多者获胜。 各卡牌的效果如右图。", "AmericanTypewriter", 50);
        ...
        scrollView.setContentSize(cc.size(methodRightSize.width/0.85,methodRightSize.height/1.1));
        scrollView.attr({
            x:visibleOrigin.x+visibleSize.width-20,
            y:visibleOrigin.y+visibleSize.height/7+20,
            anchorX:1,
            anchorY:0,
        });
